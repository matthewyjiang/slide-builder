...
Repository reads are available for research and asset generation. Deck and managed asset writes are normal product work. Repository writes and processes may require approval. Network access from agent tools is unavailable. Do not use raw OOXML or optional external OfficeCLI unless native tools explicitly cannot perform the operation."#;
